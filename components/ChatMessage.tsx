...
  // NOTE: We must ensure handleQuizSubmit logic uses the current message.quizData,
  // not a stale state from component remounts, so we only track selection/submission state locally.
  const [selectedOption, setSelectedOption] = useState<number | null>(null);
  const [quizSubmitted, setQuizSubmitted] = useState(false);
  const t = i18n[language];

  // Reset local quiz state when the message ID changes (i.e., new question arrives)
  React.useEffect(() => {
    // If the message is a model response with quiz data, reset selection state for the new question
    if (message.role === 'model' && message.quizData) {
        setSelectedOption(null);
        setQuizSubmitted(false);
    }
}, [message.id, message.role, message.quizData]);
...
    if (quizSubmitted || !message.quizData || !onQuizAnswer) return;
    setSelectedOption(index);
    setQuizSubmitted(true);
    const isCorrect = index === message.quizData.correctAnswerIndex;
    // NOTE: The onQuizAnswer prop signature is (answerText: string) in App.tsx, not (answerText: string, isCorrect: boolean)
    // We only pass the selected answer text, App.tsx handles the prompt generation.
    onQuizAnswer(message.quizData.options[index], isCorrect);
  };
...
      const isCorrect = selectedOption === correctAnswerIndex;
      ...
      // --- START OF FIX: Clean the content before displaying it ---
      let feedbackText = message.content;
      
      // 1. Remove JSON code fences if the backend missed them
      feedbackText = feedbackText
        .replace(/```json[\s\S]*?```/g, '') 
        .replace(/```[\s\S]*?```/g, '')
        .trim();
        
      // 2. Remove any remaining bare JSON object containing "question"
      const jsonRegex = /[\s\S]*(\{[\s\S]*"question"[\s\S]*\})[\s\S]*/; 
      if (jsonRegex.test(feedbackText)) {
          // This ensures that the whole JSON object is replaced with an empty string
          feedbackText = feedbackText.replace(jsonRegex, '').trim();
      }
      
      // Check if we have valid, non-placeholder feedback text left
      const hasFeedback = feedbackText && feedbackText.trim() !== "" && feedbackText.trim() !== "Here is your next question:";
      // --- END OF FIX ---

      return (
        <div className="flex flex-col items-center w-full">
           {/* Display ONLY the cleaned feedback text (if it exists) */}
           {hasFeedback && (
            <div className="w-full mb-4 p-3 bg-white/50 dark:bg-black/20 rounded-lg text-slate-700 dark:text-slate-300 text-sm">
              <ReactMarkdown>{feedbackText}</ReactMarkdown> {/* Use CLEANED text here */}
            </div>
          )}
...
            <div className="text-base md:text-lg font-bold">
                <span className={riskColor}>{riskLevel.toUpperCase()}</span> {language === 'my' && t.risk}
            </div>
          </div>
          
